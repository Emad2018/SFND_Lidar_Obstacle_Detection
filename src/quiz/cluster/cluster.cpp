...
void Proximity(const std::vector<float> &point, std::vector<int> &cluster,
  ...
  std::vector<int> found = tree->search(point, distanceTol);

  for (int found_index = 0; found_index < found.size(); found_index++) {

    if (std::find(processed.begin(), processed.end(), found[found_index]) ==
        processed.end()) {

      Proximity(point, cluster, tree, distanceTol, processed,
                found[found_index]);
    }
  }
}
...
euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree,
  ...
  for (int index = 0; index < points.size(); index++) {
    ...
    if (std::find(processed.begin(), processed.end(), index) ==
        processed.end()) {

      std::vector<int> cluster;
      Proximity(points[index], cluster, tree, distanceTol, processed, index);
